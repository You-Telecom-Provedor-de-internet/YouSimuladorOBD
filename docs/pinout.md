...
| Sinal | GPIO / pino fisico | Funcao | Origem da confirmacao | Observacoes eletricas | Classificacao |
|---|---|---|---|---|---|
| `GND_CHASSIS` | `J1 pin 4` | retorno de terra | `docs/02` + `docs/03` + `docs/15` + `netlist-rev-a.csv` | ligado ao `GND` comum da placa | confirmado na documentacao/EDA existente |
| `GND_SIGNAL` | `J1 pin 5` | retorno de terra | `docs/02` + `docs/03` + `docs/15` + `netlist-rev-a.csv` | ligado ao `GND` comum da placa | confirmado na documentacao/EDA existente |
| `CANH` | `J1 pin 6` | barramento CAN high | `docs/02` + `docs/03` + `docs/15` + `netlist-rev-a.csv` | vai ao `U2 CANH`; roteamento pareado com `CANL` | confirmado na documentacao/EDA existente |
| `KLINE` | `J1 pin 7` | barramento K-Line | `docs/02` + `docs/03` + `docs/13` + `netlist-rev-a.csv` | vai ao `U3 pin 6`; idle alto em `+12V` | confirmado na documentacao/EDA existente |
| `CANL` | `J1 pin 14` | barramento CAN low | `docs/02` + `docs/03` + `docs/15` + `netlist-rev-a.csv` | vai ao `U2 CANL`; roteamento pareado com `CANH` | confirmado na documentacao/EDA existente |
| `L_LINE` | `J1 pin 15` | `L-Line` | `docs/02` + `docs/03` + `docs/13` | documentada como `NC` nesta revisao | confirmado na documentacao/EDA existente |
| `+12V_OBD` | `J1 pin 16` | alimentacao de entrada | `docs/02` + `docs/03` + `docs/15` + `netlist-rev-a.csv` | fonte primaria do sistema | confirmado na documentacao/EDA existente |
...
| `U3 LO` | `L9637D pin 2` | saida nao usada | `docs/13` | `NC` nesta revisao | confirmado na documentacao/EDA existente |

## Programacao e debug

| Sinal | GPIO / pino fisico | Funcao | Origem da confirmacao | Observacoes eletricas | Classificacao |
|---|---|---|---|---|---|
| `USB_DEVKIT` | conector USB do modulo | gravacao do firmware e serial monitor | `docs/17` + `README.md` | precisa ficar acessivel na borda da carrier | confirmado na documentacao/EDA existente |
...
1. `docs/07-ui-controls.md` nao pode ser usada sozinha como fonte de comportamento da UI local.
